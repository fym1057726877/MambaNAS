...
def get_default_transform(image_size=128):
    transform_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.4460], std=[0.0390])
    ])
...
class VERA_PV220(Dataset):
    def __init__(self, transform=None):
        super().__init__()
        path_dir_train = os.path.join(project_path, 'datasets', 'VERA_PV220', "train")
        path_dir_test = os.path.join(project_path, 'datasets', 'VERA_PV220', "test")
...
def getVERADataLoader(image_size=128, batch_size=11, train_imgs_per_class=5, test_imgs_per_class=2,
    ...
    train_dataset = VERA_PV220(transform=transform_train)
    test_dataset = VERA_PV220(transform=transform_test)
    train_img_indexs = []
    test_img_indexs = []
    val_img_indexs = []
...
def compute_mean_variance(channel=1):
    train_loader = getVERADataLoader(image_size=128, batch_size=1, train_imgs_per_class=5,
                                     test_imgs_per_class=2, val_imgs_per_class=3)[0]
